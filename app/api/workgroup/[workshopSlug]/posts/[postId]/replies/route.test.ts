...
describe("POST .../replies", () => {
  ...
  it("returns 401 when not signed in", async () => {
    ...
    const res = await POST(req, {
      params: Promise.resolve({ workshopSlug: "w", postId: "p1" }),
    });
  ...
  it("returns 404 when post missing", async () => {
    ...
    const req = new NextRequest("http://localhost/api/workgroup/w/posts/missing/replies", {
      method: "POST",
      body: JSON.stringify({ body: "ok" }),
    });
    const res = await POST(req, {
      params: Promise.resolve({ workshopSlug: "w", postId: "missing" }),
    });
